        ...
        <div className="joinOuterContainer" style={{ flexDirection: 'row' }}>
            ...
            {isLoading ?
                ...
                : <div className="vuc">
                    <h1>Voted Users</h1>
                    <h3>{err}</h3>
                    {isData.length !== 0 ? <ul className='ulvuc'>{isData.map((option) => {
                        return <li key={id} style={{ margin: '15px' }}>{option}</li>;
                    })}</ul> : <h4>No users have voted yet</h4>}
                    ...
                    <div>
                        <button onClick={() => {
                            readXlsxFile(selectedFile).then((rows) => {
                                const res = rows.slice(1).map(val => val[0]).sort();
                        ...
                        }} className='btnn' style={{ marginTop: '20px', fontSize: '17px' }}>Submit</button>
                        {/* <DropDown /> */}
                    </div>
                </div>}
            {notVoted.length > 0 && selectedFile ? <div style={{ background: 'white', borderRadius: '20px' }} className='vuc'>
            ...
            {unauthorisedVoters.length > 0 && selectedFile ? <div style={{ background: 'white', borderRadius: '20px' }} className='vuc'>
            ...
            {unauthorisedVoters.length === 0 && submit && selectedFile ? <div style={{ background: 'white', borderRadius: '20px', height: '150px', padding: '10px' }} className='vuc'>
                <h2>No unauthorised voters</h2>
            </div> : ''}
            {notVoted.length === 0 && submit && selectedFile ? <div style={{ background: 'white', borderRadius: '20px', height: '150px', padding: '10px' }} className='vuc'>
